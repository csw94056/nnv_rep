...
figure('Name', 'ReLU computation 2 layers')
myReLU.plot_set(I, 'I poly');
myReLU.plot_set(S, 'I star');

Rp = myReLU.layerReach(I, W2_1, b2, 'approx');
myReLU.plot_set(Rp,'R poly after W2_1 approx');
Rs = myReLU.layerReach(S, W2_1, b2, 'approx');
myReLU.plot_set(Rs,'R star after W2_1 approx');

Rp = myReLU.layerReach(Rp, W3_2, b3, 'approx');
myReLU.plot_set(Rp,'R poly after W3_2 approx');
Rs = myReLU.layerReach(Rs, W3_2, b3,'approx');
myReLU.plot_set(Rs,'R star after W3_2 approx');
...
for i=1:4
    div = 2^i;
    tic
    R = myReLU.set_div(I, div, div);
    time = toc;
    myReLU.plot_set(R,'I poly div');
    R = myReLU.layerReach(R, W2_1, b2, 'approx');
    str = sprintf('R poly div after W2_1 approx (%f sec)', time);
    myReLU.plot_set(R,str);
    tic
    R = myReLU.set_div(S, div, div);
    myReLU.plot_set(R, 'I star div');
    time = toc;
    R = myReLU.layerReach(R, W2_1, b2,'approx');

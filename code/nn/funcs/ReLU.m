...
% %             check_upper_a = upper_a{len}
% %             L = Wl(:,2:end)*lower_a{len}(:,2:end)
% %             U = Wu(:,2:end)*upper_a{len}(:,2:end);
% %             lower_a{len}(:,2:end) = L;
% % %             upper_a{len}(:,2:end) = U;
            
%             R = AbsStar(In.V, In.C, In.d, lower_a, upper_a, lb, ub, In.iter);
            R = In;
        end
        
        function [R,W] = approxSingleStepReLU_absstar(I, i, l, u, W)
            % Zonotope approximation with decreasing function of lamda
            % I: intermediate input set
            % i: index of current neuron
            % lb: lower-bound of x[i]

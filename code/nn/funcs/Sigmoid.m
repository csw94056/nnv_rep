classdef Sigmoid
    ...
    methods(Static)
        function [R, ct] = FNN_reach_BFS(I, W, b, func)
            ...
            if ~strcmp(method, 'tansig') && ~strcmp(method, 'logsig')
    ...
    function S = stepSigmoid(I, i, func)
        ...
        if lb == ub % no approximation need
            ...
            V = I.V;
            V(i, 2:I.nVar+1) = 0; % reset old center and basis vectors at neuro i to zero
            V(i, 1) = yl; % sigmoid(x(i)) = yl = yu
            S = Star(V, I.C, I.d);

        else % need approximation

            % compute new center and basis vectors
            V = I.V;
            ...
            m = I.nVar; % number of predicate variables
            % new constraints
            % case 1
            if lb >= 0
                ...
                % y2 <= y'(u) * (x - u) + yu
                C2 = [-dyu*I.V(i, 2:m+1) 1];
                d2 = dyu*(I.V(i, 1) - ub) + yu;

                % constraint (3): a_new <= y3 <-> C3*a <= d3
                % y3 <= y'(1) * (x - 1) + yl
                C3 = [-dyl*(I.V(i, 2:m+1)) 1];
                d3 = dyl*(I.V(i, 1) - lb) + yl;
